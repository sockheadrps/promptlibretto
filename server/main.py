...
def _build_engine() -> PromptEngine:
    ...
    provider = MockProvider() if USE_MOCK else OllamaProvider(base_url=OLLAMA_URL)
    return PromptEngine(
        config=config,
...
async def iterate(body: IterateBody):
    ...
    if body.mode == "compact":
        original_config = eng.config
        if body.compact_config:
            eng.update_config(eng.config.merged_with(body.compact_config))
        try:
            result = await eng.generate_once(
                EngineRequest(
                    mode="compact_turn",
            ...
            if body.compact_config:
                eng.update_config(original_config)
...
async def generate(body: GenerateRequest):
    ...
    if body.config_overrides:
        eng.update_config(eng.config.merged_with(body.config_overrides))
    try:
        result = await eng.generate_once(
            EngineRequest(
                mode=body.mode,
